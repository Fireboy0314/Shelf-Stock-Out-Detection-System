...
```text
我们使用 void_detection 数据集训练 YOLO 缺货检测模型。
我们使用 MIMEX 数据集训练 ResNet 商品粗分类模型。
```
...
```text
系统由 YOLO11 detector 和 ResNet18 classifier 组成。
YOLO11 负责定位缺货空位。
ResNet18 负责判断附近商品类别。
```
...
```text
YOLO test metrics
YOLO sample prediction images
ResNet classification report
ResNet normalized confusion matrix
GUI screenshot
```
...
```text
1. 系统不能识别具体 SKU，只能识别粗类别。
2. ResNet 是单标签分类模型，一张 crop 只输出一个主类别。
3. 如果空位附近有多种商品，分类结果可能不稳定。
4. MIMEX 是单商品图片，和真实货架截图存在 domain gap。
5. YOLO 检测的是可见空位，不一定等于真实库存缺货。
```
...
```text
1. 运行 smoke test，确认代码没问题。
2. 在 Colab 上正式训练 YOLO。
3. 在 Colab 上正式训练 ResNet。
4. 下载或同步训练好的模型。
5. 本地运行 GUI，生成报告截图。
6. 整理 metrics、混淆矩阵和样例图。
7. 写最终报告。
```
